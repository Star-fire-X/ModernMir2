...
int main() {
  ...
  auto struck_timed_pose = struck_animations.pose_for(1);
  assert(struck_timed_pose.has_value());
  assert(struck_timed_pose->body_index == struck_pose->body_index);
  struck_animations.update(struck_world, 9081);
  struck_timed_pose = struck_animations.pose_for(1);
  assert(struck_timed_pose.has_value());
  assert(struck_timed_pose->body_index ==
         appearance.body_offset +
  ...
  assert(bubble_pose->overlays[1].frame_index == 800);
  ...
  assert(fire_hit_pose->overlay_count == 1);
  assert(fire_hit_pose->overlays[0].archive == ArchiveId::magic);
  assert(fire_hit_pose->overlays[0].frame_index == 3390 + 20);
