...
| P0-6 玩家/怪物 `RM_STRUCK` 延迟 | 客户端可见 `SM_STRUCK` 对玩家和怪物主目标来自 `SendDelayMsg(..., 200ms)`; 刺杀/半月/十字的二级 `DirectAttack` 可见受击为 500ms。非玩家目标另有即时 `SendMsg(RM_STRUCK)` 供怪物 AI/RunMsg 内部反应, 不是 socket 可见包。 | `ObjBase.pas:5261-5265`, `ObjBase.pas:5449`, `ObjBase.pas:5542`, `ObjBase.pas:12572-12614`; 详见 `attack_call_chain.md:§6.6`, `struck_delay_player_vs_monster.json` | C++ 当前统一 frame-end 派发存在时序风险; 需要明确保留 200ms/500ms 两种可见受击延迟, 并区分怪物内部反应消息。 | PR2/PR9 trace 对齐 | no |
